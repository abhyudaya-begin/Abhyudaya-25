...
import { removeEvent } from "../../Redux/EventSlice"; // Adjust path if needed
...
const RegisteredEvents = () => {
  ...
  // Get pending & paid events (grouped by transaction ID)
  ...
  console.log({ processingEvents, eventsPending, eventsPaid }); // Debugging
  ...
  // Helper function to render event groups
  const renderEventBox = (events, title, isProcessing = false) => (
    <div className="bg-white/10 p-4 sm:p-5 rounded-xl mt-4">
      ...
      {events.length > 0 ? (
        ...
        <p className="text-white/50 text-sm mt-2">No {title.toLowerCase()} events.</p>
      )}
    </div>
  );
  ...
  return (
    <div className="w-full bg-white/10 rounded-2xl p-4 sm:p-5">
      ...
      {/* Processing Events (with Remove Button) */}
